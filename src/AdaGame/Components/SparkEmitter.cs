...
namespace AdaGame
{
    public class SparkEmitter : DrawableGameComponent
    {
        ...
        public SparkEmitter(GameStarter game)
            : base(game)
        {
            _game = game;
        }
        ...
        private void ParticleInit(TextureRegion2D textureRegion)
        {
            _particleEffect = new ParticleEffect(autoTrigger: false)
            {
                ...
                {
                    new ParticleEmitter(textureRegion, 600, TimeSpan.FromSeconds(2),
                        Profile.Point())
                    {
                        AutoTrigger = false,
                        ...
                        Modifiers =
                        {
                            new AgeModifier
                            {
                                Interpolators =
                                {
                                    ...
                                    {
                                        StartValue = HslColor.FromRgb(Color.Yellow),//new HslColor(0.33f, 0.5f, 0.5f), 
                                        EndValue = HslColor.FromRgb(Color.YellowGreen)
                                    }
                                }
                            },
